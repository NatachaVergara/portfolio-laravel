        ...
        @foreach ($skills as $skill)
            <div class="card col-1 mx-1" style="width: 14rem;">
                <img src="{{ asset('upload/skills/' . $skill->path) }} " class="img-fluid my-3 mx-auto" style="width: 2rem;"
                    alt="...">
                <form method="POST" action="{{ route('skills.destroy', $skill->id) }}" class='d-flex justify-content-around align-items-center'>
                    @csrf
                    @method('DELETE')
                    <x-adminlte-button class="btn-md mb-2 " type="submit" theme="outline-danger" icon="fas fa-xs fa-trash" />
                </form>
